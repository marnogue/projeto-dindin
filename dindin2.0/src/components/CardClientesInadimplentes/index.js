...
export default function CardClientesInadimplentes({ ordenacao }) {
  ...
  useEffect(() => {
    try {
      async function listarClientes() {
        ...
        const responseData = await response.data.clientesInadimplentes.rows;
    ...
    <div className="conteudo-tabela-cliente">
      ...
      {filtro ?
        ...
        : clientes.map((cliente) => (
          <>
            {!idClientes.includes(cliente.id) && idClientes.push(cliente.id) && <div className="clientes-usuarios" key={cliente.id}>
